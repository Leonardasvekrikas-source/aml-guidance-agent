...
## Phase B — a report that contradicted its own table

The contextual-retrieval comparison was generated by reusing the ablation
script written for the embedding-model swap. Its "Reading this" section was
hardcoded and said:

> **BM25 is the control.** It does not use embeddings, so it must be identical
> across runs. If it moves, the experiment leaked and nothing else in the table
> can be trusted.

Directly above that sentence, the table showed BM25 moving by +0.033.

**Why the note was right once and wrong here.** Swapping the embedding model
leaves the indexed text untouched, so BM25 genuinely is a control and a move
would mean the experiment leaked. Contextual retrieval changes the indexed text
itself — that is the whole technique — so BM25 moving is the *intended effect*.
The same sentence is a valid control argument in one experiment and a false
alarm in the other.

**Why it matters more than a typo.** The report would have told a reader that
its own results were untrustworthy, in a repository whose entire argument is
that the numbers are trustworthy. Anyone who read it carefully would have
concluded either that the experiment was broken or that the author had not read
his own output.

**Fix.** The interpretation is passed in per comparison (`--reading-file`)
rather than hardcoded, and the module docstring now states plainly that whether
BM25 is a control depends on what the ablation changed. The generic default
makes no claim about controls at all.

**Caught by** reading the generated artifact instead of trusting that a working
script produced a correct report. The script worked perfectly; the prose it
carried was wrong.

---

## Phase B — the cost estimate was 39% low

Contextualising 2,864 chunks was estimated at $3.38 on Haiku and cost **$4.71**.

The estimator assumes 4 characters per token and a fixed 40 output tokens per
chunk. Both are optimistic: regulatory PDFs tokenise worse than 4 characters
per token because of citations, numbers and legal formatting, and the situating
sentences ran longer than 40 tokens.

**It still did its job.** The point of the estimate was never precision — it was
to distinguish $3 from $30 before committing, and it did that correctly. A
naive implementation of the same technique, one call per chunk with the whole
document uncached, would have cost roughly ten times as much.
